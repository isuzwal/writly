...
const Authlesspage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/post/user-noauth/${username}`, {
          credentials: "include",
        });
  ...
  if (loading) {
    return <div className="text-white h-screen bg-maincolor flex justify-center items-center text-2xl">Getting user profile</div>;
  }
